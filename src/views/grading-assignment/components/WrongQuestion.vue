    ...
    <div class="content">
      ...
      <ul class="list">
        ...
        >
          ...
          <div>
            {{
              (student.trueRate != undefined &&
                Math.round(100 - student.trueRate) + '%') ||
                '--'
            }}
          </div>
          <div v-if="student.homeworkSubmittime">
            {{ student.homeworkSubmittime | formatDate('yy/MM/dd hh:mm') }}
          </div>
          <div v-else>--</div>
          <div>
  ...
  methods: {
    handleSort(n, s) {
      ...
      if (n == 'trueRate') {
        this.list.sort((a, b) => {
          return s == 'Asc'
            ? (a[n] == undefined ? 0 : 100 - Number(a[n])) -
                (b[n] == undefined ? 0 : 100 - Number(b[n]))
            : (b[n] == undefined ? 0 : 100 - Number(b[n])) -
                (a[n] == undefined ? 0 : 100 - Number(a[n]))
        })
      }
