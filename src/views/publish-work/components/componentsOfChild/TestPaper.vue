<template>
  <div class="work-type-conent-test">
  ...
  methods: {
    ...
    selectAll(val) {
      ...
      this.workbookPageQuestionnumList.forEach(items => {
        ...
        this.addWorkbookPageQuestionnumList.push(items)
      })
      if (this.addWorkbookPageQuestionnumAllList.length === 0) {
        ...
        if (this.isAllChecked) {
          ...
          var arr = [...this.addWorkbookPageQuestionnumAllList]
          this.addWorkbookPageQuestionnumAllList.forEach(item => {
            if (this.addWorkIdList.includes(item.questionId)) {
    ...
    tapNav(i, name) {
      var self = this
      document.querySelectorAll('.test-paper')[0].scrollTop =
        document.getElementById(name).offsetTop - 300
      setTimeout(function() {
        self.tapType = i
      }, 70)
    },
    getPaperQuestion(id, list = []) {
      var self = this
      var arr = []
      // this.workbookPageQuestionnumList = []
      this.addWorkIdList = []
      this.$store.dispatch('publish/getPaperQuestion', id).then(data => {
        ...
        this.$nextTick().then(() => {
          ...
          var bescroll = $('.test-paper').scrollTop()
          $('.test-paper').scroll(function() {
            var afscroll = $('.test-paper').scrollTop()
